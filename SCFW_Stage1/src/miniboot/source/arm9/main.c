...
int main(void) {
    ...
    // debug output.
	uint32_t pressed_keys = ~REG_KEYINPUT;
#ifndef NDEBUG
#ifdef DEBUG
    debugEnabled = true;
#else
    debugEnabled = (pressed_keys & (KEY_L | KEY_R)) == (KEY_L | KEY_R);
#endif
#endif
	...
	if((pressed_keys & (KEY_L | KEY_R)) == KEY_L) {
		readFromSD = false;
	} else {
		readFromSD = true;
	}
	...
	if(readFromSD) {
		...
		dldi_patch_relocate(&_io_dldi_stub, 4, DLDI_BACKUP);
		dprintf("Mounting FAT filesystem... ");
		checkErrorFatFs("Could not mount FAT filesystem", f_mount(&fs, "fat:", 1));
		dprintf("OK\n");
	...
	if(readFromSD) {
		...
		readBundledNdsFromFlash(NDS_HEADER, 0, sizeof(nds_header_t));
	}
    ...
    // Load the ARM7 binary.
    {
		...
		if(readFromSD) {
			...
			readBundledNdsFromFlash((void*) (in_arm7_ram ? 0x2000000 : NDS_HEADER->arm7_start), NDS_HEADER->arm7_offset, NDS_HEADER->arm7_size);
		}
    ...
    // Load the ARM9 binary.
    {
		...
		if(readFromSD) {
			...
			readBundledNdsFromFlash((void*)NDS_HEADER->arm9_start, NDS_HEADER->arm9_offset, NDS_HEADER->arm9_size);
		}
